...
-- Schema "template": authentication / authorization core tables (self-contained,
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS template.md_role (
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS template.md_user (
    ...
    kd_role         VARCHAR(10)  NOT NULL REFERENCES template.md_role(kd_role) ON UPDATE CASCADE ON DELETE RESTRICT,
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS template.d_password_history (
    id              BIGSERIAL    PRIMARY KEY,
    uid_user_system VARCHAR(36)  NOT NULL REFERENCES template.md_user(uid_user_system) ON UPDATE CASCADE ON DELETE CASCADE,
    password_hash   VARCHAR(150) NOT NULL,
    created_at      TIMESTAMP    NOT NULL DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_d_password_history_uid_created
    ON template.d_password_history (uid_user_system, created_at DESC);

-- ----------------------------------------------------------------------------
-- d_password_reset_token: forgot / admin / expired-change one-time tokens
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS template.d_password_reset_token (
    id              BIGSERIAL    PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_d_password_reset_token_uid
    ON template.d_password_reset_token (uid_user_system);

-- ----------------------------------------------------------------------------
-- md_menu: application menu tree
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS template.md_menu (
    kd_menu     VARCHAR(10)  PRIMARY KEY,
    nm_menu     VARCHAR(100) NOT NULL,
    icon_menu   TEXT,
    link_menu   VARCHAR(250),
    kd_parent   VARCHAR(10)  REFERENCES template.md_menu(kd_menu) ON UPDATE CASCADE ON DELETE SET NULL,
...
-- DT=Delete, VW=View)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS template.md_action (
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS template.d_action_menu (
    kd_menu     VARCHAR(10) NOT NULL REFERENCES template.md_menu(kd_menu) ON UPDATE CASCADE ON DELETE CASCADE,
    kd_action   VARCHAR(10) NOT NULL REFERENCES template.md_action(kd_action) ON UPDATE CASCADE ON DELETE CASCADE,
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS template.d_permissions (
    kd_permission VARCHAR(20) PRIMARY KEY,
    kd_role       VARCHAR(10) NOT NULL REFERENCES template.md_role(kd_role) ON UPDATE CASCADE ON DELETE CASCADE,
    kd_menu       VARCHAR(10) NOT NULL REFERENCES template.md_menu(kd_menu) ON UPDATE CASCADE ON DELETE CASCADE,
    kd_action     VARCHAR(10) NOT NULL REFERENCES template.md_action(kd_action) ON UPDATE CASCADE ON DELETE CASCADE,
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS template.d_permissions_private (
    kd_permission   VARCHAR(20) PRIMARY KEY,
    uid_user_system VARCHAR(36) NOT NULL REFERENCES template.md_user(uid_user_system) ON UPDATE CASCADE ON DELETE CASCADE,
    kd_menu         VARCHAR(10) NOT NULL REFERENCES template.md_menu(kd_menu) ON UPDATE CASCADE ON DELETE CASCADE,
    kd_action       VARCHAR(10) NOT NULL REFERENCES template.md_action(kd_action) ON UPDATE CASCADE ON DELETE CASCADE,
...
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS template.d_application (
...
DROP VIEW IF EXISTS template.v_legacy CASCADE;
DROP VIEW IF EXISTS template.v_user CASCADE;

-- v_user: user + role, convenient for login / profile lookups
CREATE OR REPLACE VIEW template.v_user AS
SELECT
    u.uid_user_system,
    u.nama,
    u.email,
...
SELECT
    ...
    a.nm_action,
    a.deskripsi
FROM template.d_permissions p
JOIN template.md_menu m ON m.kd_menu = p.kd_menu
JOIN template.md_action a ON a.kd_action = p.kd_action
WHERE m.status = 'A'
ORDER BY m.urut_global, m.level, m.urut;

-- v_menu_private: per-user permission overrides
CREATE OR REPLACE VIEW template.v_menu_private AS
SELECT
    p.kd_permission,
    p.uid_user_system,
    p.kd_menu,
    ...
    a.nm_action,
    a.deskripsi
FROM template.d_permissions_private p
JOIN template.md_menu m ON m.kd_menu = p.kd_menu
JOIN template.md_action a ON a.kd_action = p.kd_action
WHERE m.status = 'A'
ORDER BY m.urut_global, m.level, m.urut;
