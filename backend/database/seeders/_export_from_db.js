 ...
 * (schema `template`) into `database/seeders/*.sql`.
 *
...
async function getRows(client, tableName, columns, orderByColumns) {
  ...
  const res = await client.query(
    `SELECT ${colsSql} FROM template.${escapeIdent(tableName)}${orderSql}`,
  );
  return res.rows;
}
...
function buildUpsert(sqlTable, columns, pkColumns, rows) {
  ...
  if (nonPkColumns.length === 0) {
    ...
    return [
      `INSERT INTO template.${escapeIdent(sqlTable)} (${colsSql}) VALUES`,
  ...
  try {
    ...
    for (const t of tables) {
      ...
      const headerLines = [
        ...
        `-- Seed: ${t.filename}`,
        `-- Generated at: ${nowIso}`,
        `-- From: ${process.env.DB_HOST || 'localhost'} / ${process.env.DB_NAME || 'template'} (schema template)`,
        `-- Table: template.${t.table}`,
        `-- ================================================================`,
    ...
    for (const st of serialTargets) {
      const seqRes = await client.query(
        ...
        [`template.${st.table}`, st.col],
      );
      const seqName = seqRes.rows[0]?.seq;
      if (!seqName) continue;

      const maxRes = await client.query(
        `SELECT MAX(${escapeIdent(st.col)})::bigint AS maxval FROM template.${escapeIdent(st.table)}`,
      );
      const maxval = maxRes.rows[0]?.maxval;
      const nextBase = (maxval === null || maxval === undefined) ? 0 : Number(maxval);

      seqLines.push(
        `SELECT setval('${seqName}', COALESCE((SELECT MAX(${escapeIdent(st.col)}) FROM template.${escapeIdent(st.table)}), ${nextBase}), true);`,
      );
    }
